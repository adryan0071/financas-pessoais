...
export function MainDashboard() {
  ...
  const totalBudgeted = getTotalBudgeted(currentDate.getFullYear(), currentDate.getMonth() + 1)
  const totalSpent = getTotalSpent(currentDate.getFullYear(), currentDate.getMonth() + 1)
  ...
  const budgetAlerts = budgetsWithSpent.filter(budget => 
    budget.status === BUDGET_STATUS.WARNING || budget.status === BUDGET_STATUS.EXCEEDED
  )

  // Metas com progresso
  const goalsWithProgress = getGoalsWithProgress()
  const activeGoals = goalsWithProgress.filter(goal => goal.status === 'active').slice(0, 3)
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case BUDGET_STATUS.ON_TRACK: return 'text-green-600'
      case BUDGET_STATUS.WARNING: return 'text-yellow-600'
      case BUDGET_STATUS.EXCEEDED: return 'text-red-600'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Aqui está um resumo das suas finanças em {format(currentDate, "MMMM 'de' yyyy", { locale: ptBR })}
          </p>
        </div>
      ...
      {showNotifications && (budgetAlerts.length > 0 || activeGoals.some(g => g.progress.isOverdue)) && (
        <Alert className="border-yellow-200 bg-yellow-50">
          <Bell className="h-4 w-4" />
          <AlertDescription>
            <div className="space-y-1">
              <p className="font-medium">Você tem notificações importantes:</p>
              {budgetAlerts.length > 0 && (
                <p className="text-sm">
                  • {budgetAlerts.length} orçamento(s) próximo(s) do limite ou ultrapassado(s)
                </p>
              )}
              {activeGoals.some(g => g.progress.isOverdue) && (
                <p className="text-sm">
                  • {activeGoals.filter(g => g.progress.isOverdue).length} meta(s) com prazo vencido
                </p>
              )}
            </div>
          </AlertDescription>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              de {formatCurrency(getTotalTargeted())} (meta)
            </p>
            <Progress 
              value={getTotalTargeted() > 0 ? (totalSaved / getTotalTargeted()) * 100 : 0} 
              className="mt-2 h-2" 
            />
          </CardContent>
        </Card>
                ...
                <div key={transaction.id} className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className={`w-2 h-2 rounded-full ${
                      transaction.type === TRANSACTION_TYPES.INCOME ? 'bg-green-500' : 'bg-red-500'
                    }`} />
                    <div>
              ...
              {activeGoals.map((goal) => (
                <div key={goal.id} className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={goal.progress.isOverdue ? 'destructive' : 'default'}>
              ...
              {budgetsWithSpent.slice(0, 4).map((budget) => (
                <div key={budget.id} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {budget.status === BUDGET_STATUS.ON_TRACK && <CheckCircle className="h-3 w-3 text-green-600" />}
                    {budget.status === BUDGET_STATUS.WARNING && <AlertTriangle className="h-3 w-3 text-yellow-600" />}
                    {budget.status === BUDGET_STATUS.EXCEEDED && <Clock className="h-3 w-3 text-red-600" />}
                  </div>
                </div>
              ))}
