...
import { Loader2, Wallet, Building2, CreditCard, PiggyBank, TrendingUp } from 'lucide-react'
...
export function AccountForm({ account, onSave, onCancel }) {
  ...
  const { addAccount, updateAccount, isLoading, error, clearError } = useAccounts()
  ...
  const onSubmit = async (data) => {
    try {
      const accountData = {
        ...
        creditLimit: data.type === ACCOUNT_TYPES.CREDIT ? parseFloat(data.creditLimit) || 0 : undefined
      }
      ...
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {errors.type && (
              <p className="text-sm text-destructive">Tipo de conta é obrigatório</p>
            )}
          </div>
              ...
              <Input
                ...
                {...register('balance', {
                  ...
                  min: {
                    ...
                    message: watchedType === ACCOUNT_TYPES.CREDIT 
          ...
          <div className="flex space-x-4">
            <Button type="submit" className="flex-1" disabled={isLoading}>
              {isLoading ? (
                ...
                isEditMode ? 'Atualizar Conta' : 'Criar Conta'
              )}
            </Button>
